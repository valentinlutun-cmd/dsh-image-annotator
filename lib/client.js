/**
 * dsh-image-annotator — client half.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---------------------------------------------------------------- CSS
		const CSS_TAG = "@deepseek-ai/dsh-image-annotator/client.css";
		const CSS = `
...
`;
		if (typeof document !== "undefined" && document.querySelector(`style[data-plugin-css="${CSS_TAG}"]`) === null) {
			...
			tag.dataset.plugin = "dsh-image-annotator";
		...
		function findDraftFor(ctx, src) {
			try {
				...
				const ids = [];
				// dsh 0.1.7 sessions.list is a snapshot store of
				// { ids, byId, phase, projectionsBySession }. The session shown in
				// the main view is the one with retainedBy.mainView > 0 (same
				// signal the product's own workspace/open-in-app code uses), so
				// try it first; fall back to every known session id.
				for (const id of Object.keys(byId)) {
					const row = byId[id];
					if (row && row.retainedBy && (row.retainedBy.mainView ?? 0) > 0) { ids.unshift(id); break; }
				}
				const known = Array.isArray(snap.ids) ? snap.ids : Object.keys(byId);
				for (const id of known) if (id !== void 0 && !ids.includes(id)) ids.push(id);
				for (const id of ids) {
		...
		function attachLightbox(ctx, dialog, img) {
			...
			function copyImage() {
				...
				} : null).then((blob) => navigator.clipboard.write([new ClipboardItem({ "image/png": blob })])).catch((err) => console.warn("dsh-image-annotator: copy failed", err));
			}
			...
			function scan() {
				...
				for (const dialog of document.querySelectorAll('[role="dialog"]')) {
					...
					try {
						...
						delete dialog.__igl;
						console.warn("dsh-image-annotator: attach failed", err);
						continue;
					}
		...
		// --------------------------------------------------------------- apply
		function apply(ctx) {
			ctx.effect(() => installDetector(ctx), "dsh-image-annotator: lightbox augmentor");
		}
		exports.apply = apply;
